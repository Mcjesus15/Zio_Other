...
int bcmsdh_probe(struct device *dev)
{
	...
	resume_bcmsdh_probe = 1;
	printk("%s:%d, resume_bcmsdh_probe = %d\n", __func__, __LINE__, resume_bcmsdh_probe);
...
#if defined(OOB_INTR_ONLY)
extern void msmsdcc_wifi_hclk_pclk_on(unsigned int on); //Bruno, 20100806, Bug605, Implement OOB interrupt
static irqreturn_t wlan_oob_irq(int irq, void *dev_id)
{
	dhd_pub_t *dhdp;

	dhdp = (dhd_pub_t *)dev_get_drvdata(sdhcinfo->dev);
   msmsdcc_wifi_hclk_pclk_on(1); //Bruno, 20100806, Bug605, Implement OOB interrupt
...
int bcmsdh_register_oob_intr(void * dhdp)
{
	...
	error = request_irq(sdhcinfo->oob_irq, wlan_oob_irq, IRQF_SHARED | IRQF_TRIGGER_RISING, //Bruno
		"bcmsdh_sdmmc_oob", dhdp); // Bruno, 20100806, Bug605, Implement OOB interrupt
