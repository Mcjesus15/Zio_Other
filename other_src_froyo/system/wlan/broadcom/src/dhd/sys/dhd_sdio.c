...
#include <rtecdc.h> //vivien@cci modify
#endif /* BCMEMBEDIMAGE */
...
#define PMU_MAX_TRANSITION_DLY 100000000 // vivien@cci add
#endif
...
clkwait:
	...
	if ((bus->dhd->busstate == DHD_BUS_DOWN) || bcmsdh_regfail(sdh)) {
		DHD_ERROR(("%s: failed backplane access over SDIO, halting operation\n",
		           __FUNCTION__));
		printk("%s, CountBusDown=%d\n", __func__, CountBusDown);
		if (CountBusDown > 5) {
		    if(bcmsdh_regfail(sdh)) {
			DHD_ERROR(("%s: failed backplane access over SDIO, halting operation --- bcmsdh_regfail \n",
				    __FUNCTION__));
		    }
		    else {
			bus->dhd->busstate = DHD_BUS_DOWN;
	...
	uint16 func, uint bustype, void *regsva, osl_t * osh, void *sdh)
{
	...
	resume_dhdsdio_probe = 1;
	printk("%s:%d, resume_dhdsdio_probe = %d\n", __func__, __LINE__, resume_dhdsdio_probe);
...
dhd_bus_devreset(dhd_pub_t *dhdp, uint8 flag)
{
	...
	if (flag == TRUE) {
		...
		To prevent SDIO from sleeping when Driver was restarted,
		...
		*/
		...
		DHD_TRACE(("%s: SDIO_TIMER resettimg 3000[ms] \n", __FUNCTION__));
		dhd_sdclk_timer_driver_intial_setting( dhdp );
