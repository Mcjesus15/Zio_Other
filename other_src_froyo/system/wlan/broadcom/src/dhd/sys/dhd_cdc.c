...
#define htod32(i) i
	...
	if (dhd && dhd->up) {
		if(!dhd_os_proto_block(dhd))
			printk("%s: dhd_os_proto_block() return 0\n", __FUNCTION__);
		
		if (value) {
			dhdcdc_set_ioctl(dhd, 0, WLC_SET_PM,
				(char *)&power_mode, sizeof(power_mode));
			printk("%s: WLC_SET_PM = %d (PM_MAX=1)\n", __FUNCTION__, power_mode);
			...
			dhdcdc_set_ioctl(dhd, 0, WLC_SET_PM, (char *)&power_mode,
				sizeof(power_mode));
			printk("%s: WLC_SET_PM = %d (PM_FAST=2;PM_OFF=0)\n", __FUNCTION__, power_mode);
			/* disable pkt filter */
			enable_parm.id = htod32(100);
...
//E: Bruno, 20100716, TK11330, CPU sleep control during Wi-Fi on
	...
	/* Set btc_params 6 for co-existence issue. vivien@cci */
	{
	    char reg6_value[8] = { 6, 00, 00, 00, 0x0a, 0x00, 0x00, 0x00 };
	    bcm_mkiovar("btc_params", reg6_value, 8, iovbuf, sizeof(iovbuf));
	    dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, iovbuf, sizeof(iovbuf));
	}
