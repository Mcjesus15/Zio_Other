...
void enable_dev_wlc_ioctl(void)
{
    dev_wlc_ioctl_off = 0;
}
...
)
{
	...
	if (country_offset != 0) {
		...
		snprintf(country_code, 3, "US"); // force set country code to US. J07's request
		/* 11 for North America, 14 for Japan, 13 for most of world */
		/* vivien@cci take it off for J07's request
		if (strncmp(country_code, "11", 2) == 0)
		    snprintf(country_code, 3, "US");
		else if (strncmp(country_code, "14", 2) == 0)
		    snprintf(country_code, 3, "JP");
		else if (strncmp(country_code, "13", 2) == 0)
		    snprintf(country_code, 3, "CN"); */
		
		if ((error = dev_wlc_ioctl(dev, WLC_SET_COUNTRY,
			&country_code, sizeof(country_code))) >= 0) {
			p += snprintf(p, MAX_WX_STRING, "OK");
			WL_TRACE(("%s: set country %s OK\n", __FUNCTION__, country_code));
			printk("%s: set country %s OK\n", __FUNCTION__, country_code);
			goto exit;
		}
	}

	WL_ERROR(("%s: set country %s failed code %d\n", __FUNCTION__, country_code, error));
	printk("%s: set country %s failed code %d\n", __FUNCTION__, country_code, error);
...
)
{
	...
	if (strnicmp((char *)&powermode_val, "1", strlen("1")) == 0) {
		...
		dev_wlc_ioctl(dev, WLC_SET_PM, &pm, sizeof(pm)); //vivien@cci add
#ifdef COEX_DHCP
...
static int set_ap_cfg(struct net_device *dev, struct ap_profile *ap)
{
	...
	if (ap_cfg_running == FALSE) {
		...
		if ((res = dev_wlc_ioctl(dev, WLC_UP, &updown, sizeof(updown))) < 0) {
			WL_ERROR(("%s fail to set apsta - WLC_UP\n", __FUNCTION__));
			goto fail;
		}
...
)
{
	...
	if (dwrq->length && extra) {
	    ...
	    if (strnicmp(extra, "SCAN-ACTIVE", strlen("SCAN-ACTIVE")) == 0) {
		    ...
		    int channel_num;
		    char *p = extra;
		    union iwreq_data *wrqu = (union iwreq_data *)dwrq;
		    dev_wlc_ioctl(dev, WLC_GET_CHANNEL, &channel_num, 4);
		    printk("we got channel number: %d\n", channel_num);
		    p += snprintf(p, MAX_WX_STRING, "SCAN-CHANNELS %d", channel_num);
		    wrqu->data.length = p - extra + 1;
	    }
		else if (strnicmp(extra, "GETPM", strlen("GETPM")) == 0) {
			int pm_mode;
			char *p = extra;
			union iwreq_data *wrqu = (union iwreq_data *)dwrq;
			dev_wlc_ioctl(dev, WLC_GET_PM, &pm_mode, 4);
			printk("we got pm mode: %d\n", pm_mode);
			p += snprintf(p, MAX_WX_STRING, "PM MODE %d", pm_mode);

			wrqu->data.length = p - extra + 1;

	    } else if (strnicmp(extra, "GETBCNPRD", strlen("GETBCNPRD")) == 0) {
			int bcn_period;
			char *p = extra;
			union iwreq_data *wrqu = (union iwreq_data *)dwrq;
			int ret;
			...
			ret = dev_wlc_ioctl(dev, WLC_GET_BSSID, &bssid, ETHER_ADDR_LEN);
			if (ret != 0)
				...
				*(uint32*)buf = htod32(WLC_IOCTL_SMLEN);
				...
				if (dtoh32(bi->version) == WL_BSS_INFO_VERSION) {
					bcn_period = bi->beacon_period;
					printk("beacon period = %d\n", bcn_period);
					p += snprintf(p, MAX_WX_STRING, "Beacom_Period %d", bcn_period);
		...
		else if (strnicmp(extra, "GETDTIM", strlen("GETDTIM")) == 0) {
			int dtim;
			char *p = extra;
			union iwreq_data *wrqu = (union iwreq_data *)dwrq;
			int ret;
			...
			if (ret != 0)
				...
				*(uint32*)buf = htod32(WLC_IOCTL_SMLEN);
				...
				if (dtoh32(bi->version) == WL_BSS_INFO_VERSION) {
					dtim = bi->dtim_period;
					p += snprintf(p, MAX_WX_STRING, "DTIM_Period %d", dtim);

					wrqu->data.length = p - extra + 1;
					printk("dtim period = %d\n", dtim);
				}
				else
					printk("wrong version\n");
...
int wl_iw_get_BSSID(struct net_device *dev, char *buf)
{
    return  dev_wlc_ioctl(dev, WLC_GET_BSSID, (void*)buf, ETHER_ADDR_LEN);
}
