...
#define LOG_TAG "wpa_supplicant.c" 
...
static void wpa_supplicant_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_s->reassociate = 1;
	wpa_s->assoc_timeout_count++; //penguin add for wpa2-eap reconnected failed issue
...
void wpa_supplicant_cancel_auth_timeout(struct wpa_supplicant *wpa_s)
{
	...
	wpa_s->assoc_timeout_count = 0; //penguin add; for wpa2-eap reconnect failed issue
			      ...
			      struct wpa_scan_res *bss, struct wpa_ssid *ssid)
{
	...
	if (bss && (wpa_scan_get_vendor_ie(bss, WPA_IE_VENDOR_TYPE) ||
		...
		try_opportunistic = ssid->proactive_key_caching &&
			(ssid->proto & WPA_PROTO_RSN);
		/* penguin, modify for wpa2-eap reconnect failed issue */
		if (wpa_s->assoc_timeout_count < 2) {
			LOGE("%s, wpa_s->assoc_timeout_count=%d", __func__, wpa_s->assoc_timeout_count);
			wpa_printf(MSG_WARNING, "%s, wpa_s->assoc_timeout_count=%d"
				, __func__, wpa_s->assoc_timeout_count);
			if (pmksa_cache_set_current(wpa_s->wpa, NULL, bss->bssid,
					    wpa_s->current_ssid,
					    try_opportunistic) == 0)
					eapol_sm_notify_pmkid_attempt(wpa_s->eapol, 1);
		} else {
			LOGE("%s, wpa_s->assoc_timeout_count=%d, clear current pmksa_cache", __func__, wpa_s->assoc_timeout_count);
			wpa_printf(MSG_WARNING, "%s, wpa_s->assoc_timeout_count=%d, clear current pmksa_cache"
				, __func__, wpa_s->assoc_timeout_count);
			pmksa_cache_clear_current(wpa_s->wpa);
			eapol_sm_notify_pmkid_attempt(wpa_s->eapol, 0);
		}

		/*penguin, modify end for wpa2-eap reconnect failed issue */
