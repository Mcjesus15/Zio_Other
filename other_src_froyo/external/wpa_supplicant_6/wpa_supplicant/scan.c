...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	while (ssid) {
#if 0 /*penguin, force scan for every remembered network */
		if (!ssid->disabled &&
		    (ssid->scan_ssid || wpa_s->conf->ap_scan == 2)) {
#else
		if (!ssid->disabled) {
#endif
			wpa_printf(MSG_ERROR, "ssid->disabled = %d, scan_ssid = %d, ap_scan = %d",
				ssid->disabled, ssid->scan_ssid, wpa_s->conf->ap_scan);
			break;
		}
		ssid = ssid->next;
	}
